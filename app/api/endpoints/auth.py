...
async def forgot_password(body: ForgotPasswordRequest, db: DBSession) -> dict:
    ...
    if raw_token:
        try:
            # Gap 5 confirmed: fragment (#) keeps token out of server logs and Referer headers
...
async def reset_password_endpoint(body: ResetPasswordRequest, db: DBSession) -> dict:
