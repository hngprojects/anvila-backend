...
def mock_reset_password_request():
    return ResetPasswordRequest(
        reset_token="mock_reset_tokenmock_reset_tokenmock_reset_tokenmock_reset_token",
...
def test_request_reset_link_user_not_found(
    mock_db_session, mock_request_password_service
):
    mock_request_password_service.fetch.side_effect = HTTPException(
        status_code=404, detail="User not found"
    )

    reset_email = RequestEmail(email="unknown@gmail.com")
    response = client.post(
        "/api/v1/auth/forgot-password", json={"email": reset_email.email}
    )
...
def test_reset_password_invalid_token(
    mock_db_session, mock_reset_password_request, mock_request_password_service
):
    mock_request_password_service.update.side_effect = HTTPException(
        status_code=400, detail="reset token invalid"
    )

    response = client.patch(
        "/api/v1/auth/reset-password", json=mock_reset_password_request.dict()
    )
